...
### 4.1 Mounting a Partition by Device Path

```bash
sudo mount -t ext4 /dev/sdb1 /mnt/data
```

### 4.2 Mounting by UUID
...
- **Danger**: Running `mount -a` after editing `/etc/fstab` will **not** catch syntax errors or missing paths for filesystems that are already mounted. A broken `/etc/fstab` can cause Linux systems to drop into an emergency maintenance shell on next boot.
- **Upstream Recommendation**: Always verify `/etc/fstab` using `findmnt --verify` rather than `mount -a`.
